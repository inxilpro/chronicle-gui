...
public enum Collector {
    ...
    /// one from the provider's current call when needed.
    public static func ensureCurrentSession(
        store: ChronicleStore, provider: any CallProvider
    ) throws -> SessionRecord {
        if let session = try store.currentSession() {
            // Existing active/finalizing session writes remain available when
            // the provider is between calls or temporarily unavailable.
            if let callId = try? provider.currentCall(), callId != session.id {
                return try store.createOrResumeSession(callId: callId)
            }
            return session
        }
        guard let callId = try provider.currentCall() else {
            throw ChronicleError("no active Chronicle session; join a Tuple call and open Chronicle first")
        }
        return try store.createOrResumeSession(callId: callId)
    }

    /// `startSessions` controls whether a detected call may create or resume
    /// a session. The CLI keeps the default — running the skill is an
    ...
    public static func collectOnce(
        store: ChronicleStore, provider: any CallProvider, timeout: String,
        startSessions: Bool = true
    ) throws {
        let currentCall: Result<String?, Error>
        do {
            currentCall = .success(try provider.currentCall())
        } catch {
        ...
        case .success(.some(let callId)):
            ...
            if let active = session, active.state == .active {
                if active.id == callId {
                    ...
                    try provider.collect(store: store, session: active, timeout: timeout)
                } else {
                    // The provider moved on to a different call. Keep draining
                    // the tracked call's cursor for its call_ended and run the
        ...
        case .success(.none):
            ...
            // A reader scoped to the stable call ID receives the provider's
            // explicit call_ended status after the current-call endpoint
            // becomes empty.
            if let active = session, active.state == .active {
                try provider.collect(store: store, session: active, timeout: timeout)
        ...
        case .failure(let error):
            ...
            if let active = session {
                try store.setSourceState(
                    sessionId: active.id, source: provider.id, status: "error", detail: message)
            } else {
